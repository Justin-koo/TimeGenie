...
class AdminSiteMiddleware:
    ...
    def __scall__(self, request):
        # Check if the path is accessible for students or if the user is a staff member or admin
        if not request.path.startswith(reverse('login')) and not any(request.path.startswith(path) for path in self.student_accessible_paths):
            if not (request.user.is_authenticated and (request.user.is_staff or request.user.is_superuser or request.user.groups.filter(name='Students').exists())):
